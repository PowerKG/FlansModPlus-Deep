...
public class ItemGun extends Item implements IPaintableItem
{
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List lines, boolean advancedTooltips)
	{
		...
		lines.add("§f§n===========§7§lETO§f§n===========");
		lines.add("§f§l群号807687935");
		lines.add("           §d按§f§n§lshift§d查看更多信息");
		if (this.type.oneHanded) {
		      lines.add("§e[§d§l可双持§e]");
		    }
		if(this.type.canShootUnderwater){
			lines.add("§a[§b§l水中射§e]");
		}
		if(this.type.consumeGunUponUse){
			lines.add("§c[§8§l一次性§c]");
		}
		if(this.type.showCrosshair){
			lines.add("§b[§f§l无准心§b]");
		}
		if(this.type.shield){
			lines.add("§d[§6§l含盾牌 §d]");
		}
		if (this.type.mode == EnumFireMode.BURST) {
		      lines.add("§a[§b§l三连发§a]");
		    } else if (this.type.mode == EnumFireMode.FULLAUTO) {
		      lines.add("§a[§b§l全自动§a]");
		    } else if (this.type.mode == EnumFireMode.SEMIAUTO) {
		      lines.add("§a[§b§l半自动§a]");
		    } else if (this.type.mode == EnumFireMode.MINIGUN) {
		      lines.add("§a[§b§l转管式§a]");
		    }
		lines.add("§f§n===========§7§lETO§f§n===========");
		if (this.type.ammo.size() > 0)
	    {
	      lines.add("§d↓§6§l可装填弹药§d↓");
	      for (ShootableType shootable : this.type.ammo)
	      {
	        String line = shootable.name;
	        
	        float baseDamage = this.type.damage * shootable.damageVsLiving;
	        line = line + "§f输出 " + baseDamage;
	        if (this.type.numBullets > 1) {
	          line = line + " §4*" + this.type.numBullets;
	        }
	        for(int i = 0; i < type.getNumAmmoItemsInGun(stack); i++)
			{
				...
				if(bulletStack != null && bulletStack.getItem() instanceof ItemBullet)
				{
					...
					String line1 = bulletType.name + " " + (bulletStack.getMaxDamage() - bulletStack.getItemDamage()) + "/" + bulletStack.getMaxDamage();
					lines.add("§f§l当前弹药:§c§l"+line1);
				}
			}
	        
	      }
		
	      lines.add("§8§n=========§8☠ETO科技☠§8§n=========");
		...
		if(!GameSettings.isKeyDown(shift))
		{
			...
			lines.add("§b〄 §e§l瞄准精度:§a" + roundFloat(type.getSpread(stack), 2));
			lines.add("§a◎§e§l换弹速度:§a" + "\u00a77: " + roundFloat(type.getReloadTime(stack) / 20, 2) + "s");
			lines.add("§d↓§6§l配件列表§d↓");
			boolean empty = true;
			for(AttachmentType attachment : type.getCurrentAttachments(stack))
			{
				String line = attachment.name;
				lines.add("§e§l『§c§l"+line+"§e§l]");
				if(line != null)
					empty = false;
			}
