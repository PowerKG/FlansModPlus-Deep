...
public class ItemGun extends Item implements IPaintableItem {
	...
	/**
	 * Get the bullet item stack stored in the gun's NBT data (the loaded magazine /
	 * bullets)
	 */
	...
	/**
	 * Set the bullet item stack stored in the gun's NBT data (the loaded magazine /
	 * bullets)
	 */
...
//			lines.add("\u00a79Mode" + "\u00a77: \u00a7f" + type.getFireMode(stack).toString().toLowerCase());
	...
	public void onUpdateClient(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		if (entity instanceof EntityPlayer && ((EntityPlayer) entity).inventory.getCurrentItem() == itemstack) {
			...
			if (soundDelay <= 0 && type.idleSound != null) {
				PacketPlaySound.sendSoundPacket(entity.posX, entity.posY, entity.posZ, type.idleSoundRange,
			...
			if (FMLClientHandler.instance().getClient().currentScreen != null) {
				if (FlansModClient.currentScope != null) {
					...
					FlansMod.getPacketHandler()
							.sendToServer(new PacketGunSpread(itemstack, type.getDefaultSpread(itemstack)));
				}
			}
			...
			// Else do shoot code
					...
					if (data.offHandGunSlot != 0) {
						...
						if (offHandGunStack != null && offHandGunStack.getItem() instanceof ItemGun) {
							...
							if (offHandGunType.usableByPlayers) {
								// If we are using a burst mode gun, and there is burst left to be done, try to
								// do it
								if (offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.BURST
									...
									if ((offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.FULLAUTO
											...
											&& !leftMouseHeld && lastLeftMouseHeld) // Full auto. Send released mouse
																					// packet
									{
					...
					if (type.getFireMode(itemstack) == EnumFireMode.BURST && data.burstRoundsRemainingRight > 0) {
						...
						if (rightMouseHeld && !lastRightMouseHeld) {
							FlansMod.getPacketHandler().sendToServer(
									new PacketGunFire(false, true, player.rotationYaw, player.rotationPitch));
							if (clientSideShoot(player, itemstack, type, false))
								player.inventory.setInventorySlotContents(player.inventory.currentItem, null);
						}
						if ((type.getFireMode(itemstack) == EnumFireMode.FULLAUTO
								|| type.getFireMode(itemstack) == EnumFireMode.MINIGUN) && !rightMouseHeld
								&& lastRightMouseHeld) // Full auto. Send released mouse packet
						{
							FlansMod.getPacketHandler().sendToServer(
									new PacketGunFire(false, false, player.rotationYaw, player.rotationPitch));
						}
						if ((type.getFireMode(itemstack) == EnumFireMode.FULLAUTO
								|| type.getFireMode(itemstack) == EnumFireMode.MINIGUN) && rightMouseHeld) {
							if (clientSideShoot(player, itemstack, type, false))
				...
				if (FlansModClient.aimType == AimType.TOGGLE) {
					if (!offHandFull
							...
							&& Mouse.isButtonDown(FlansModClient.aimButton.getButton()) && FlansModClient.scopeTime <= 0
							&& FMLClientHandler.instance().getClient().currentScreen == null) {
						if (FlansModClient.currentScope == null) {
							/*
							 * if(type.allowNightVision) isNightVision = true; if(type.allowSlow) isSlow =
							 * true;
							 */
							...
							FlansMod.getPacketHandler()
									.sendToServer(new PacketGunState(FlansModClient.currentScope != null));
						} else {
					...
					if (!offHandFull
							...
							&& /* FlansModClient.scopeTime <= 0 && */ FMLClientHandler.instance()
		...
		if ((FlansModClient.shootTime(left) <= 0 && sprinting && FlansModClient.zoomProgress > 0.5F)
			...
			for (int i = 0; i < gunType.getNumAmmoItemsInGun(stack); i++) {
				...
				if (bulletStack != null && bulletStack.getItem() != null
					...
					if (bulletStack.getMaxDamage() - bulletStack.getItemDamage() == 1
							&& (gunType.model != null ? gunType.model.slideLockOnEmpty : false))
						onLastBullet = true;
					hasAmmo = true;
					break;
				}
			...
			if (hasAmmo) {
				 ...
				 * Ultimate FlansModClient.playerRecoilPitch += gunType.getRecoilPitch(stack);
				 * FlansModClient.playerRecoilYaw += gunType.getRecoilYaw(stack); } else {
				 * FlansModClient.playerRecoilPitch += gunType.getRecoilPitch(stack) -
				 * gunType.decreaseRecoilPitch; FlansModClient.playerRecoilYaw +=
				 * gunType.getRecoilYaw(stack) / gunType.decreaseRecoilYaw; <<<<<<< HEAD }
				 */
				if (left)
	...
	public void onUpdateServer(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		...
		// TODO Temp marker
		/*
		 * //If enabled a speed nerf will be applied for each gun after gunCarryLimt
		 * starting at 0.6 and decreasing by 0.1 for each additional
		 * if(FlansMod.gunCarryLimitEnable && entity instanceof EntityPlayer) {
		 * EntityPlayer player = (EntityPlayer) entity; int gunCount = 0; for (int slot
		 * = 0; slot < 9; slot++) { ItemStack itemInSlot =
		 * player.inventory.getStackInSlot(slot); ItemStack current =
		 * player.inventory.getCurrentItem(); if(itemInSlot != null &&
		 * itemInSlot.getItem() instanceof ItemGun) { gunCount++; } } if(gunCount >
		 * FlansMod.gunCarryLimit) { float movementSpeed = MathUtils.clampf((float) (0.1
		 * - (0.04f + (0.010f * (gunCount - FlansMod.gunCarryLimit)))), 0.01f, 0.1f);
		 * //player.capabilities.setPlayerWalkSpeed(walkSpeed);
		 * //player.jumpMovementFactor = 0.002F; FlansMod.getPacketHandler().sendTo(new
		 * PacketUpdateSpeed(0.01F, 0.002F), (EntityPlayerMP) player); } else {
		 * //player.capabilities.setPlayerWalkSpeed(0.1F); //player.jumpMovementFactor =
		 * 0.02F; //(default) FlansMod.getPacketHandler().sendTo(new
		 * PacketUpdateSpeed(0.1F, 0.02F), (EntityPlayerMP) player); } }
		 */
		...
		if (entity instanceof EntityPlayerMP) {
			...
			if (player.inventory.getCurrentItem() != itemstack) {
				// If the player is no longer holding a gun, emulate a release of the shoot
				// button
		...
		if (pEnt instanceof EntityPlayer) {
			...
			if (type.lockOnToLivings || type.lockOnToMechas || type.lockOnToPlanes || type.lockOnToPlayers
				...
				for (Object obj : player.worldObj.loadedEntityList) {
					...
					double dXYZ = Math.sqrt((entity.posX - player.posX) * (entity.posX - player.posX)
							+ (entity.posY - player.posY) * (entity.posY - player.posY)
							+ (entity.posZ - player.posZ) * (entity.posZ - player.posZ));
					Vector3f relPosVec = new Vector3f(entity.posX - player.posX, entity.posY - player.posY,
					...
					if (angle < Math.toRadians(type.canLockOnAngle) && dXYZ < type.maxRangeLockOn) {
						...
						if ((type.lockOnToMechas && entity instanceof EntityMecha)
							...
							// if(entity instanceof EntityMecha || entity instanceof EntityVehicle || entity
							// instanceof EntityPlane || entity instanceof EntityPlayer || entity instanceof
							// EntityLivingBase)
				...
				if (closestEntity != null && lockOnSoundDelay <= 0 && !player.worldObj.isRemote
						&& player.getCurrentEquippedItem() != null) {
					if (player.getCurrentEquippedItem().getItem() instanceof ItemGun) {
						ItemGun itemGun;
						itemGun = (ItemGun) player.getCurrentEquippedItem().getItem();
						PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 10, player.dimension,
								itemGun.type.lockOnSound, false);
						// PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, 100,
						// player.dimension, itemGun.type.shootSound, false);
						lockOnSoundDelay = type.lockOnSoundTime;
						if (closestEntity instanceof EntityDriveable
								&& ((EntityDriveable) closestEntity).getDriveableType().hasFlare) {
							...
							PacketPlaySound.sendSoundPacket(closestEntity.posX, closestEntity.posY, closestEntity.posZ,
									entityDriveable.getDriveableType().lockedOnSoundRange, closestEntity.dimension,
			...
			/*
			 * //TODO; Add scope attachment override to enable NV for add-on NV scopes //If
			 * player is holding gun, apply modifiers below
			 * if(player.getCurrentEquippedItem() != null &&
			 * player.getCurrentEquippedItem().getItem() instanceof ItemGun) { ItemGun
			 ...
			 * 
			 * } AttachmentType scope = itemGun.type.getScope(itemstack); //IScope scope =
			 * itemGun.type.getCurrentScope(itemstack); //System.out.println(scope); //Apply
			 * night vision while scoped if attachment.hasNightVision = True if(scope !=
			 * null && scope.hasNightVision && FlansModClient.currentScope != null) {
			 * player.addPotionEffect(new PotionEffect(Potion.nightVision.id, -1, 0));
			 * //System.out.println("1"); } //Apply a penalty to jumpMovement equal to the
			 * moveSpeed penalty (0.5 moveSpeed = 0.5 jumpDistance)
			 * if(itemGun.type.moveSpeedModifier != 1F) { player.jumpMovementFactor = 0.0F;
			 * } else player.jumpMovementFactor = 0.02F; }
			 */

			// if(data.lastMeleePositions == null || data.lastMeleePositions.length !=
			// type.meleeDamagePoints.size())
			// {
			// data.lastMeleePositions = new Vector3f[type.meleeDamagePoints.size()];
			// for(int j = 0; j < type.meleeDamagePoints.size(); j++)
			// data.lastMeleePositions[j] = new Vector3f(player.posX, player.posY,
			// player.posZ);
			// }
			// Melee weapon
						...
						if (!hits.isEmpty()) {
							...
							for (BulletHit bulletHit : hits) {
								...
								if (bulletHit instanceof PlayerBulletHit) {
									...
									if (FlansMod.DEBUG)
										world.spawnEntityInWorld(new EntityDebugDot(world, new Vector3f(
												data.lastMeleePositions[k].x + dPos.x * playerHit.intersectTime,
									...
									if (FlansMod.DEBUG)
										world.spawnEntityInWorld(new EntityDebugDot(world, new Vector3f(
												data.lastMeleePositions[k].x + dPos.x * entityHit.intersectTime,
		...
		if ((left && data.shootTimeLeft <= 0) || (!left && data.shootTimeRight <= 0)) {
			...
			if (bulletStack == null) {
				...
				if (reload(gunStack, gunType, world, entityplayer, false, left)) {
					...
					// (int)gunType.getReloadTime(gunStack);
					...
					FlansMod.getPacketHandler().sendTo(new PacketReload(left, reloadCount, (int) reloadTime),
							entityplayer);
					// Play reload sound
					...
					PacketPlaySound.sendSoundPacket(entityplayer.posX, entityplayer.posY, entityplayer.posZ,
							type.reloadSoundRange, entityplayer.dimension, gunType.clickSoundOnEmpty, true);
					canClick = false;
...

			{
				...
				if (!world.isRemote) {
					GunTryFireEvent.Fire tryFireEvent = new GunTryFireEvent.Fire(entityplayer.getUniqueID(), gunType);
					FlansModDeep.event_bus.post(tryFireEvent);
					if (tryFireEvent.isCanceled())
						return gunStack;
				}
	...
	public boolean reload(ItemStack gunStack, GunType gunType, World world, Entity entity, IInventory inventory,
		...
		if (gunType.deployable)
			return false;
		// If you cannot reload half way through a clip, reject the player for trying to
		// do so
		...
		for (int i = 0; i < gunType.getNumAmmoItemsInGun(gunStack); i++) {
			...
			// If there is no magazine, if the magazine is empty or if this is a forced
			// reload
			if (bulletStack == null || bulletStack.getItemDamage() == bulletStack.getMaxDamage() || forceReload) {
				// Iterate over all inventory slots and find the magazine / bullet item with the
				// most bullets
				int bestSlot = -1;
				...
				// If there was a valid non-empty magazine / bullet item somewhere in the
				// inventory, load it
				if (bestSlot != -1) {
					// TODO
					...
					// Unload the old magazine (Drop an item if it is required and the player is not
					// in creative mode)
					...
					// The magazine was not finished, pull it out and give it back to the player or,
					// failing that, drop it
	...
	private void shoot(ItemStack stack, GunType gunType, World world, ItemStack bulletStack, EntityPlayer entityPlayer,
		...
		for (int i = 0; i < type.getNumAmmoItemsInGun(stack); i++) {
			...
			if (bulletStacks[i] != null && bulletStacks[i].getItem() instanceof ItemBullet
					&& (bulletStacks[i].getMaxDamage() - bulletStacks[i].getItemDamage() == 1))
				lastBullet = true;
		}
		...
		if (soundDelay <= 0 && gunType.shootSound != null) {
			...
			// world.playSoundAtEntity(entityplayer, type.shootSound, 10F, type.distortSound
			// ? 1.0F / (world.rand.nextFloat() * 0.4F + 0.8F) : 1.0F);
			String soundToPlay = null;
		...
		if (!world.isRemote && bulletStack.getItem() instanceof ItemShootable) {
			...
			spread *= (entityPlayer.isSneaking() ? 0.7F : 1F);
			float speed = gunType.getBulletSpeed(stack);
			float damage = gunType.getDamage(stack);

			GunFireEvent fireEvent = new GunFireEvent(entityPlayer.getUniqueID(), gunType, shootableType, numBullets,
					spread, speed, damage);
			...
			FlansMod.packetHandler.sendTo(new PacketGunRecoil(gunType.getRecoilPitch(stack),
					gunType.getRecoilYaw(stack), gunType.decreaseRecoilPitch, gunType.decreaseRecoilYaw),
				...
				if (look.sideHit == 1) {
					...
					if (!world.isRemote) {
						...
						if (isSolid(world, i, j, k)
								...
								&& (world.getBlock(i + (playerDir == 1 ? 1 : 0) - (playerDir == 3 ? 1 : 0), j + 1,
	...
	public Multimap getAttributeModifiers(ItemStack stack) {
		...
		map.put(SharedMonsterAttributes.movementSpeed.getAttributeUnlocalizedName(),
				new AttributeModifier(field_111210_e, "MovementSpeed", type.getMovementSpeed(stack) - 1F, 2));
