...
public class GunType extends PaintableType implements IScope {
	...
	/**
	 * The amount to recoil the player's view by when firing a single shot from this
	 * gun
	 */
	...
	/**
	 * The secondary function of this gun. By default, the left mouse button
	 * triggers this
	 */
	...
	public boolean lockOnToPlanes = false, lockOnToVehicles = false, lockOnToMechas = false, lockOnToPlayers = false,
	...
	// Shields
	// A shield is actually a gun without any shoot functionality (similar to knives
	// or binoculars)
	// and a load of shield code on top. This means that guns can have in built
	// shields (think Nerf Stampede)
	...
	/**
	 * Whether to distort the sound or not. Generally only set to false for looping
	 * sounds
	 */
	...
	// Looping sounds
	/**
	 * Whether the looping sounds should be used. Automatically set if the player
	 * sets any one of the following sounds
	 */
	...
	// Deployable Settings
	/**
	 * If true, then the bullet does not shoot when right clicked, but must instead
	...
	/**
	 * Various deployable settings controlling the player view limits and standing
	 * position
	 */
	public float standBackDist = 1.5F, topViewLimit = -60F, bottomViewLimit = 30F, sideViewLimit = 45F,
			pivotHeight = 0.375F;
	...
	// Attachment settings
	/**
	 * If this is true, then all attachments are allowed. Otherwise the list is
	 * checked
	 */
	...
	public boolean allowBarrelAttachments = false, allowScopeAttachments = false, allowStockAttachments = false,
			allowGripAttachments = false, allowGadgetAttachments = false, allowSlideAttachments = false,
			allowPumpAttachments = false, allowAccessoryAttachments = false;
	...
	protected void read(String[] split, TypeFile file) {
		...
		try {
			...
			else if (FMLCommonHandler.instance().getSide().isClient() && deployable
			...
			// Custom Melee Stuff
				...
				meleePath.add(new Vector3f(Float.parseFloat(split[1]) / 16F, Float.parseFloat(split[2]) / 16F,
				...
				meleeDamagePoints.add(new Vector3f(Float.parseFloat(split[1]) / 16F, Float.parseFloat(split[2]) / 16F,
						Float.parseFloat(split[3]) / 16F));
			}
			...
			// Shield settings
				...
				shieldOrigin = new Vector3f(Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F,
						Float.parseFloat(split[4]) / 16F);
				shieldDimensions = new Vector3f(Float.parseFloat(split[5]) / 16F, Float.parseFloat(split[6]) / 16F,
	...
	/**
	 * Return the currently active scope on this gun. Search attachments, and by
	 * default, simply give the gun
	 */
	...
	/**
	 * Private method for attaching attachments to a list of attachments with a
	 * nullcheck
	 */
	...
	/**
	 * Get the firing mode of a specific gun, taking into account attachments and
	 * secondary fire mode
	 */
